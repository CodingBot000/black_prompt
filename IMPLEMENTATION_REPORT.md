...
실제 Vite 프로젝트는 `frontend`에 구현했습니다.

- Vite + React + TypeScript 프로젝트 구성
- 8개 권역 지도, 권역 마커, 선택 권역 상세, 글로벌 상태, 이벤트 로그 구현
- 새 게임, 시작 권역 선택, 일 단위 자동 진행, 일시정지/재생, 1x/2x 구현
- 6개 작전, 15개 스킬, 10개 이벤트, 5개 방어 프로토콜, 4개 승리/3개 패배 엔딩 구현
- 스킬트리/지역 상세 모바일 시트/글로벌 상태/이벤트 선택/도움말/엔딩/새 게임 모달 자동 일시정지 구현
- localStorage 저장/불러오기와 자동 저장 구현
- `window.render_game_to_text`, `window.advanceTime(ms)` 브라우저 검증 훅 구현
...
- `frontend/public/world-map-placeholder.svg`: 추상 세계지도 SVG
- `frontend/src/data`: 권역/작전/스킬/이벤트/엔딩/방어 프로토콜 정적 데이터
- `frontend/src/engine`: 게임 수식과 상태 전이 순수 함수
- `frontend/src/locales`: locale catalog와 fallback
- `frontend/src/components`: 공통 UI, 레이아웃, 지도, 패널, 모달
- `frontend/src/state`: reducer 연결, loop, persistence
- `frontend/src/tests`: Vitest 테스트
...
- 스크린샷: `frontend/output/web-game/manual-gameplay.png`, `mobile-landscape.png`, `mobile-portrait.png`
